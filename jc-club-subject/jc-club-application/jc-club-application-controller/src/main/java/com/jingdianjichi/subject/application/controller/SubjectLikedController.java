...
import com.jingdianjichi.subject.application.convent.SubjectLikedDTOConverter;
...
public class SubjectLikedController {
     ...
     * 分页查询点赞列表
     */
    @RequestMapping("pageLiked")
    public Result<PageResult<SubjectLikedBO>> pageLiked(@RequestBody SubjectLikedDTO subjectLikedDTO) {

        try {
            if (log.isInfoEnabled()) {
                log.info("SubjectLikedController.pageLiked.dto:{}", JSON.toJSONString(subjectLikedDTO));
            }
            SubjectLikedBO subjectLikedBO = SubjectLikedDTOConverter.INSTANCE.convertDTOToBO(subjectLikedDTO);
            return Result.ok(subjectLikedDomainService.pageLiked(subjectLikedBO));
        } catch (Exception e) {
            log.error("SubjectLikedController.update.error:{}", e.getMessage(), e);
            return Result.fail("更新题目点赞表信息失败");
        }
